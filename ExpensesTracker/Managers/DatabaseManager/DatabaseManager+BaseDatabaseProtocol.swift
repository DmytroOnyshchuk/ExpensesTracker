...
extension DatabaseManager: BaseDatabaseProtocol {
    ...
    func saveAsync(_ object: Object, update: Realm.UpdatePolicy?) {
        realm.writeAsync {
            ...
            if let error {
                Logger.default.errorLog("Error save object \(object.className)", error: error, category: self.className)
            }
        }
    }
    ...
    func save(_ object: Object, update: Realm.UpdatePolicy?, completion: CompletionHandler) {
        guard !object.isInvalidated else {
            Logger.default.errorLog("Error save object \(object.className)", error: DBError.isInvalidate, category: self.className)
    ...
    func delete(_ object: Object, completion: CompletionHandler) {
        guard !object.isInvalidated else {
            Logger.default.errorLog("Error delete object \(object.className)", error: DBError.isInvalidate, category: self.className)
            completion(.isInvalidate)
            return
        }
        ...
        do {
            ...
            Logger.default.errorLog("Error delete object \(object.className)", error: error, category: self.className)
            completion(.other(error))
        }
    }
    ...
    func deleteBatch<T: Object>( _ objects: [T], completion: CompletionHandler) {
        ...
        do {
            ...
            Logger.default.errorLog("Error delete objects", error: error, category: self.className)
            completion(.other(error))
        }
    }
    ...
    func drop<T: Object>(_ classType: T.Type) {
        ...
        do {
            ...
            Logger.default.errorLog("Error drop objects: \(classType)", error: error, category: self.className)
        }
    }
    ...
    func deleteAll(completion: (DBError?) -> Void) {
        do {
            ...
            Logger.default.errorLog("Error delete all objects", error: error, category: self.className)
            completion(.other(error))
        }
    ...
    func getObject<T: Object, K>(_ classType: T.Type, forPrimaryKey: K) -> T? {
        ...
        guard !(object?.isInvalidated ?? false) else {
            Logger.default.errorLog("Error get object \(classType)", error: DBError.isInvalidate, category: self.className)
            return nil
        }
    ...
    func write(actionHandler: ActionHandler, completion: CompletionHandler) {
        ...
        autoreleasepool {
            do {
                ...
                dbError = .other(error)
                Logger.default.errorLog("Error write transaction", error: error, category: self.className)
            }
        }
